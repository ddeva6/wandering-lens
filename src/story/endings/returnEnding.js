...
const TICK_MS = 100;
const DRIVE_AWAY_DURATION_MS = 15000;
const DRIVE_AWAY_KMH = 60;
...
function driveAwayEast(onDone) {
  ...
  holdCameraOn(
    ...
    DRIVE_AWAY_DURATION_MS
  );

  // Drives away from wherever the jeep actually is, for a fixed duration,
  // rather than trying to "arrive" at a point computed off Amara's
  // position — the jeep could be anywhere on the map when this fires, so a
  // distance-based arrival could take far longer than the cutscene allows.
  const eastTarget = { x: jeep.position.x + 2000, z: jeep.position.z + 400 };
  const interval = setInterval(() => {
    moveToward(jeep, eastTarget, DRIVE_AWAY_KMH, TICK_MS / 1000, 3);
  }, TICK_MS);
