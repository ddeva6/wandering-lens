...
function spawnMediaJeeps(scene) {
  MEDIA_JEEP_POSITIONS.forEach((pos) => {
    ...
    [-1, 1].forEach((side) => {
      const headlight = new SpotLight(0xffffee, 2, 100, Math.PI / 6, 0.5, 1);
      headlight.position.set(side, 0, -2);
      headlight.target.position.set(side, 0, -8);
      jeep.add(headlight, headlight.target);
    });
